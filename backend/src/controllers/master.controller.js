...
const dashboard = async (req, res, next) => {
  try {
    ...
    // Audit logs scoped to master's own subtree only
    const subtreeActors = [masterId, ...agentIds, ...userIds];
    ...
    const [
      ...
      AuditLog.find({ actor: { $in: subtreeActors } })
        .sort({ createdAt: -1 })
        .limit(8)
...
const report = async (req, res, next) => {
  try {
    ...
    const [totalDeposits, totalWithdrawals, totalTx, depositSum, withdrawalSum] = await Promise.all([
