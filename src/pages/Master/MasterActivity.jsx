...
import { masterAuditLogs } from "../../api/games";
...
const MasterActivity = () => {
  ...
  const load = useCallback(() => {
    setLoading(true);
    masterAuditLogs({ page, limit: PAGE_SIZE }).then(r => {
      if (r?.success) { setLogs(r.data.logs); setTotal(r.data.total); }
  ...
  return (
    <MasterLayout pageTitle="Activity Logs">
      <div className="p-card">
        <div className="p-card-header">
          <h3>All Activity Logs</h3>
          <div className="p-search-bar">
            <input placeholder="Search actor / action..." value={search} onChange={e => { setSearch(e.target.value); setPage(1); }} style={{ width: 220 }} />
          </div>
        </div>
