...
const INIT = [
  ...
  { id: 7,  name: "Demo Fantasy", category: "Fantasy",     games: 10, enabled: true  },
  { id: 8,  name: "Evoplay",      category: "Slot",        games: 5,  enabled: true  },
];
...
const MasterProviders = () => {
  ...
  const toggle = (id) => {
    setProviders(providers.map(p => {
      if (p.id !== id) return p;
      addLog(user.username, user.role, `${p.enabled ? "Disabled" : "Enabled"} provider`, p.name);
  ...
  const handleAdd = () => {
    ...
    addLog(user.username, user.role, "Added provider", form.name);
    setShowModal(false);
