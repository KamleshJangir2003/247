...
import { agentCommissions } from "../../api/agent";
...
const AgentCommissions = () => {
  ...
  const load = useCallback(() => {
    setLoading(true);
    agentCommissions({ page, limit: PAGE_SIZE }).then(r => {
  ...
  return (
    <AgentLayout pageTitle="Commissions">
        ...
        <div className="p-card-body" style={{ padding: 0 }}>
          <div className="p-table-wrap">
            <table className="p-table">
              <thead><tr><th>#</th><th>Source User</th><th>Amount</th><th>Rate %</th><th>Status</th><th>Date</th></tr></thead>
              <tbody>
                {loading
                  ? <tr><td colSpan={6} className="p-nodata">Loading…</td></tr>
                  : rows.length === 0
                    ? <tr><td colSpan={6} className="p-nodata">No commissions found.</td></tr>
                    : rows.map((c, i) => (
                      <tr key={c._id}>
