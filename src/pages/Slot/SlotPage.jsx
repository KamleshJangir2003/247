...
import { slotProviders } from "../../data/providers";
import "../../components/gamepage/GamePage.css";
import "./SlotPage.css";

const CATS = ["All Slots", "Classic", "Video Slots", "Megaways", "Jackpot", "New", "Popular"];

const SlotPage = () => {
  const navigate = useNavigate();

  const [activeCat,      setActiveCat]      = useState("All Slots");
  ...
  const filtered = useMemo(() => {
    ...
    if (activeCat === "New")     return g.filter((x) => x.isNew);
    if (activeCat === "Popular") return g.filter((x) => x.isPopular);
    if (activeCat !== "All Slots") g = g.filter((x) => x.subCategory === activeCat);
  ...
  return (
    <div className="gpage-wrap">
      <div className="gpage-titlebar">
        ...
        <span className="gpage-titlebar-text">Slot Games</span>
        <span className="gpage-titlebar-count">{filtered.length} / {slotGames.length} Games</span>
      </div>
      ...
      <div className="gpage-body">
        <aside className="gpage-sidebar">
          ...
          <button className="slot-clear-btn" onClick={clearFilters}>✕ Clear Filters</button>
        </aside>

        <main className="gpage-main">
          <div className="slot-search-bar">
            <input
              className="slot-search-input"
              type="text"
              placeholder="Search slots by name, provider..."
              value={search}
              onChange={(e) => setSearch(e.target.value)}
            />
          </div>

          <div className="gpage-section-title">{activeCat} — Slot Games</div>

          {filtered.length === 0 ? (
            <div className="slot-empty">
              <div className="slot-empty-title">No games found</div>
              <div className="slot-empty-sub">Try another search or remove some filters.</div>
            </div>
          ) : (
