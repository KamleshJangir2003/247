...
const AdminCategories = () => {
  ...
  const toggle = (id) => setCats(cats.map(c => c.id === id ? { ...c, enabled: !c.enabled } : c));
  const handleAdd = () => {
    if (!form.name) return;
    setCats([...cats, { id: Date.now(), ...form, order: Number(form.order) || cats.length + 1, enabled: true }]);
    setShowModal(false); setForm({ name: "", icon: "", order: "" });
  };
              ...
              <tbody>
                {cats.map((c, i) => (
                  <tr key={c.id}>
                    ...
                    <td><label className="p-toggle"><input type="checkbox" checked={c.enabled} onChange={() => toggle(c.id)} /><span className="p-toggle-slider" /></label></td>
                  </tr>
                ))}
              </tbody>
            </table>
      ...
      {showModal && (
        <div className="p-modal-overlay">
          <div className="p-modal">
            <div className="p-modal-header"><h3>Add Category</h3><button className="p-modal-close" onClick={() => setShowModal(false)}><FaTimes /></button></div>
            <div className="p-modal-body">
              <div className="p-form-grid">
                ...
                <div className="p-form-group"><label>Icon</label><input value={form.icon} onChange={e => setForm({ ...form, icon: e.target.value })} placeholder="🎰" /></div>
                <div className="p-form-group"><label>Order</label><input type="number" value={form.order} onChange={e => setForm({ ...form, order: e.target.value })} /></div>
              </div>
