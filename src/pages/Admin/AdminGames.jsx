...
const AdminGames = () => {
  ...
  const handleToggle = (id, name) => { toggleGame(id); addLog(user.username, "admin", "Toggled game", name); refresh(); };
  const handleDelete = (id, name) => {
    if (!window.confirm(`Delete "${name}"?`)) return;
    deleteGame(id); addLog(user.username, "admin", "Deleted game", name); refresh();
  };
  const handleSave = () => {
    if (!form.name) return;
    if (modal === "add") { addGame({ ...form, enabled: true }); addLog(user.username, "admin", "Added game", form.name); }
    else { updateGame(modal.id, form); addLog(user.username, "admin", "Updated game", form.name); }
          ...
          <div className="p-modal">
            ...
            <div className="p-modal-body">
              <div className="p-form-grid">
                ...
                <div className="p-form-group full"><label>Image URL</label><input value={form.imageUrl} onChange={e => setForm({ ...form, imageUrl: e.target.value })} placeholder="https://..." /></div>
              </div>
