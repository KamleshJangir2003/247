...
import { createDeposit, listDeposits } from "../../api/wallet";
...
const Deposit = () => {
  ...
  const loadHistory = useCallback(() => {
    listDeposits({ limit: 50 }).then((res) => {
      if (res?.success) setHistory(res.data.deposits);
    }).catch(() => {});
  }, []);
  ...
  const handleSubmit = async () => {
    if (!utr.trim())
      return setMsg({ type: "error", text: "Please enter UTR number." });
    if (!amount || isNaN(amount) || Number(amount) < 100)
      return setMsg({ type: "error", text: "Enter valid amount (min ₹100)." });
    if (!agreed)
    ...
    const res = await createDeposit({
      ...
      metadata: { utr, method: activeTab === "auto" ? "Auto / UPI" : "Manual" },
      idempotencyKey: `${utr}-${amount}`,
    });
    setLoading(false);
    ...
    if (res?.success) {
      ...
      setMsg({ type: "error", text: res?.message || "Deposit failed." });
    }
      ...
      <div className="main-layout">
        ...
        <div className="deposit-content">
          ...
          <div className="deposit-header">
            <div className="deposit-title"><span className="dot"></span> Deposit</div>
          </div>

          {msg && <div className={`dep-msg ${msg.type}`}>{msg.text}</div>}
            ...
            <div className="right-box">
              <div className="payment-box">
                ...
                <div className="form-group">
                  ...
                  <input type="text" placeholder="Up to 12 Digit UTR Number" value={utr} maxLength={12} onChange={(e) => setUtr(e.target.value)} />
                </div>

                <div className="form-group">
                  <label>Amount*</label>
                  <input type="number" placeholder="Enter amount (min ₹100)" value={amount} onChange={(e) => setAmount(e.target.value)} />
                </div>
                ...
                <button className="submit-btn" onClick={handleSubmit} disabled={loading}>
                  {loading ? "Please wait…" : "SUBMIT DEPOSIT"}
                </button>
              </div>
            </div>
          </div>
          ...
          <div className="table-section">
            ...
            <table>
              <thead>
                <tr>
                  <th>SN.</th><th>Amount</th><th>Status</th><th>Date</th><th>UTR</th><th>Method</th>
                </tr>
              </thead>
              <tbody>
                {history.map((row, i) => (
                  <tr key={row._id}>
                    ...
                    <td>{row.metadata?.utr || "—"}</td>
                    <td>{row.metadata?.method || "Manual"}</td>
                  </tr>
                ))}
              </tbody>
            </table>
